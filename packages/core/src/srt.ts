...
const SUBTITLE_PROFILES: Record<SubtitleStyle, SubtitleProfile> = {
  short_punchy: { targetLine: 20, maxLine: 26, maxBlock: 44, minCueDurationSec: 0.8, maxCueDurationSec: 2.2 },
  clear: { targetLine: 38, maxLine: 42, maxBlock: 80, minCueDurationSec: 1, maxCueDurationSec: 4 },
  narrative: { targetLine: 32, maxLine: 38, maxBlock: 88, minCueDurationSec: 1.2, maxCueDurationSec: 4.2 },
  sales: { targetLine: 24, maxLine: 30, maxBlock: 52, minCueDurationSec: 0.8, maxCueDurationSec: 2.6 }
};
...
function enforceCueCount(chunks: string[], totalDurationSec: number, profile: SubtitleProfile): string[] {
  ...
  while (next.length < minCount) {
    const index = next.map((text, i) => ({ i, len: text.length })).sort((a, b) => b.len - a.len)[0]?.i;
    if (index === undefined) {
      break;
    }
  ...
  while (next.length > maxCount && next.length > 1) {
    const index = next.map((text, i) => ({ i, len: text.length })).sort((a, b) => a.len - b.len)[0]?.i;
    if (index === undefined) {
      break;
    }
...
function wrapTwoLines(text: string, profile: SubtitleProfile): string[] {
  ...
  if (splitIndex < 0) {
    return [normalized.slice(0, profile.maxLine).trim(), normalized.slice(profile.maxLine).trim()].filter(Boolean);
  }
...
function distributeDurations(drafts: CueDraft[], totalDurationSec: number, profile: SubtitleProfile): number[] {
  ...
  durations = durations.map((value) => Math.max(profile.minCueDurationSec, Math.min(profile.maxCueDurationSec, value)));
  let diff = totalDurationSec - durations.reduce((sum, value) => sum + value, 0);
  let guard = 0;
  while (Math.abs(diff) > 0.001 && guard < 5000) {
    guard += 1;
    if (diff > 0) {
      const expandable = durations.map((value, index) => ({ index, room: profile.maxCueDurationSec - value })).filter((item) => item.room > 0);
      ...
      const shrinkable = durations.map((value, index) => ({ index, room: value - profile.minCueDurationSec })).filter((item) => item.room > 0);
  ...
  if (Math.abs(diff) > 0.001) {
    ...
    durations[lastIndex] = Math.max(profile.minCueDurationSec, Math.min(profile.maxCueDurationSec, current + diff));
  }
  return durations;
}

export function buildTimedCues(script: string, totalDurationSec: number, subtitleStyle: SubtitleStyle = "clear"): TimedCue[] {
  ...
  return drafts.map((draft, index) => {
    ...
    cursor += duration;
    const endMs = index === drafts.length - 1 ? Math.round(totalDurationSec * 1000) : Math.round(cursor);
    return { ...draft, startMs, endMs };
  });
}
...
export function buildSrt(script: string, totalDurationSec: number, subtitleStyle: SubtitleStyle = "clear"): string {
